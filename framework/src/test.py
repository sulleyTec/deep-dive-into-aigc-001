...
def test_element_wise_add():
    ...
    arr = np.random.rand(8,32,128,128)
    brr = np.random.rand(128,128)

    numpy_result=arr+brr
    #backend = gf.BackEnd.CPU
    backend = gf.BackEnd.GPU
    print(f'{backend=}')
    gf_result = gf.element_wise_add(arr, brr, backend)
...
def test_test():
    ...
    new_shape = [N,C,H,1]
    gf.reshape(gf_mean, new_shape)
    gf.reshape(gf_var, new_shape)

    gf_tmp1 = gf.element_wise_minus(gf_input, gf_mean)
    gf_tmp2 = gf.element_wise_sqrt(gf_var+1e-3)

    gf_result = gf.element_wise_div(gf_tmp1, gf_tmp2)
    torch_tmp3 = torch_tmp1/torch_tmp2
    np.testing.assert_allclose(gf_result, torch_tmp3, rtol=1e-3)
    print(f'tmp3 pass')
...
def test_layernorm():
    ...
    #gf_result = gf.layernorm(input.detach().numpy(), [W])
    gf_result = gf.layernorm(input.detach().numpy(), [C,H,W])

    print(f'{gf_result.shape=}')
    print(f'{numpy_result.shape=}')

    np.testing.assert_allclose(gf_result, numpy_result, rtol=1e-2)

#test_layernorm()

def test_element_wise_add_scalar():
    print('test_element_wise_add')
    arr = np.random.rand(4,2,3)
    b = 1e-3

    numpy_result=arr+b
    gf_result = gf.element_wise_add_scalar(arr, b)
